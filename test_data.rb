  ...
  def test_is_station_useful
    STATIONS.each do |row|
      if CHILDREN[row["id"]].empty?
        bypass_check = Constants::STATIONS_ENABLED_ELSEWHERE.include?(row["id"])
        assert has_rail_id(row) || bypass_check,
          "Station #{row["name"]} (#{row["id"]}) is useless and should be removed"
      end
    end
  end
